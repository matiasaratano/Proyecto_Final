...
const Reserva = ({ data, navigation }) => {
  ...
  const {user, setUser, listaAReservar, setListaAReservar,refresh, setRefresh} = useContext(GlobalContext)
...
//Metodo para llamar a Api para Borrar Reserva
  const handleCancelacion = () => {
    ReservaService.deleteReserva(data.reserva.id).then(data => {
      console.log(data)
      if (data.success) {
...
//Alert de Cancelacion con 2 Botones
  const cancelarReserva = () =>
    Alert.alert('Confirmacion de Cancelacion', 'Seguro quieres Cancelar la Reserva?', [
      {
        text: 'Volver',
...
const handleVianda = ()=>{
  ReservaService.updateVianda(data.reserva.id).then(data => {
    console.log(data)
    if (data.success) {
...
//Alert de Lista de Espera con 2 Botones
  const entrarListadeEspera = () =>
    Alert.alert('Ingreso a Lista de Espera', 'Seguro quieres Ingresar a la lista de espera?', [
      {
        text: 'Volver',
  ...
  const handleListaEspera = () => {
    ReservaService.getIntoListaEspera(data.fecha,"userId").then(data => {
      console.log(data)
      if (data.success) {
      } else {
        alert(data.message)
